...
class ObjectDetection:
    def __init__(self, confidence, inference_path):
        PATH_TO_FROZEN_GRAPH=inference_path+'/frozen_inference_graph.pb'
        PATH_TO_LABELS=inference_path+'/duckie_label_map.pbtxt'
        ...
        if total_detections > 0:
            for detection in range(0, total_detections):
                if output_dict['detection_scores'][detection] > self.__confidence:
                    ...
                    detected_list.append(self.__category_index[category]['name'])
                    #row, column
                    centroids_list.append(np.array([(output_dict['detection_boxes'][detection][0]+output_dict['detection_boxes'][detection][2])/2.,(output_dict['detection_boxes'][detection][1]+output_dict['detection_boxes'][detection][3])/2.]))

        return detected_list, centroids_list
